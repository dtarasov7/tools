...
PATH_TO_FILE=$1
ES_URL=https://admin:x7RHIP@10.80.:9200
KIBANA_URL=https://admin:x7HzsIP@10.80:5601
...
}' $ES_URL/_plugins/_security/api/tenants/$tenant_name; echo
done
...
curl -k -X PUT -H 'Content-Type: application/json' -d ''$role_content'' $ES_URL/_plugins/_security/api/roles/$role_name; echo
done
...
check_user_exists=$(curl -k -s -X GET $ES_URL/_plugins/_security/api/internalusers/$user_name | grep status)
if [ $? -eq 0 ]; then
        echo "# Создание пользователя '$user_name'"
curl -k -X PUT -H "Content-Type: application/json" -d '{
        "password": "'"$user_pass"'"
}' $ES_URL/_plugins/_security/api/internalusers/$user_name; echo
else
...
curl -k -X PATCH -H "Content-Type: application/json" -d '['$mapping_content']' $ES_URL/_plugins/_security/api/rolesmapping; echo
...
curl -k -s -X POST -H "Content-Type: application/json" -H "osd-xsrf: true" -H "securitytenant: $tenant_name" -d '{
"attributes":{
  "title":"'"$index_pattern"'",
